...
export async function main() {
  ...
  if (!queuedFederated.includes('jupyter-matplotlib')) {
    try {
      let ext = require('jupyter-matplotlib/lib/plugin');
      for (let plugin of activePlugins(ext)) {
        register.push(plugin);
      }
    } catch (e) {
  ...
  if (!queuedFederated.includes('jupyterlab-system-monitor')) {
    try {
      let ext = require('jupyterlab-system-monitor');
      for (let plugin of activePlugins(ext)) {
        register.push(plugin);
      }
    } catch (e) {
